...
$('#maintenance-form').onsubmit=async e=>{e.preventDefault();try{await api('/tenant/maintenance/new',{method:'POST',body:JSON.stringify(Object.fromEntries(new FormData(e.target)))});e.target.reset();$('#maintenance-status').textContent='Request submitted';await loadMaintenance()}catch(err){fail(err)}};window.maintenanceDetail=maintenanceDetail;window.addTenantComment=addTenantComment;$('#logout').onclick=async()=>{await api('/logout',{method:'POST'});location.href='/login'};window.addEventListener('hashchange',route);route();
