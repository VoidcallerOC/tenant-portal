...
try {
  ...
  await db.transaction(async (tx) => {
    await tx.insert(organizations).values({ id: ids.organization, name: 'Harborview Property Group', slug: 'harborview', email: 'ops@harborview.example', phone: '555-0100' }).onConflictDoNothing();
    await tx.insert(users).values([
      { id: ids.admin, organizationId: ids.organization, email: 'admin@harborview.example', firstName: 'Avery', lastName: 'Stone', role: 'ADMIN', passwordHash: adminPasswordHash },
      { id: ids.manager, organizationId: ids.organization, email: 'manager@harborview.example', firstName: 'Morgan', lastName: 'Lee', role: 'MANAGER', passwordHash: managerPasswordHash },
    ...
    await tx.insert(properties).values([
      { id: ids.propertyA, organizationId: ids.organization, name: 'Harborview Lofts', addressLine1: '101 Water Street', city: 'Portland', state: 'ME', zip: '04101' },
