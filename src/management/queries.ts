...
export async function getDashboard(db: Db, organizationId: string) {
  const [propertyRows, unitRows, tenantRows, openRequestRows, recentRequests, recentTenants] = await Promise.all([
    ...
    db.select({ tenant: tenants, user: users }).from(tenants).innerJoin(users, eq(users.id, tenants.userId)).where(eq(tenants.organizationId, organizationId)),
    ...
    db.select({ request: maintenanceRequests, tenant: tenants, user: users, unit: units, property: properties }).from(maintenanceRequests)
    ...
    db.select({ tenant: tenants, user: users }).from(tenants).innerJoin(users, eq(users.id, tenants.userId)).where(eq(tenants.organizationId, organizationId)).orderBy(desc(tenants.createdAt)).limit(5),
  ]);
...
export async function getPropertyDetail(db: Db, organizationId: string, propertyId: string) {
  ...
  const tenantsForProperty = await db.select({ tenant: tenants, user: users, lease: leases, unit: units }).from(leases)
  ...
  const requests = await db.select({ request: maintenanceRequests, tenant: tenants, user: users, unit: units }).from(maintenanceRequests)
