...
export function createApp(databaseUrl?: string) {
  ...
  app.get('/admin/maintenance', requireManagement, async (req, res, next) => {
    try {
      ...
      const rows = await db.select({ request: maintenanceRequests, tenant: tenants, user: users, unit: units, property: properties })
  ...
  app.get('/admin/maintenance/:id', requireManagement, async (req, res, next) => {
    try {
      ...
      return res.json({ ...rows[0], comments: await db.select({ comment: maintenanceComments, user: users }).from(maintenanceComments).innerJoin(users, eq(users.id, maintenanceComments.userId)).where(eq(maintenanceComments.maintenanceRequestId, id)).orderBy(asc(maintenanceComments.createdAt)) });
    } catch (error) { return next(error); }
  });
  ...
  app.get('/tenant/profile', requireTenant, async (req, res, next) => { try { const context = await getTenantContext(db, (req as AuthenticatedRequest).authUser.id); return context ? res.json({ tenant: context.tenant, user: context.user }) : res.status(404).json({ error: 'Resource not found.' }); } catch (error) { return next(error); } });
